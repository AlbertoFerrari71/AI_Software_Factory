...
- update `docs/73_LAST_DEPRECATION_4_DIGIT_ARTIFACT_NAMING_STANDARD.md` when artifact naming, `LAST-*` deprecation, `max(II)` latest-artifact lookup, Bridge authority rules or migration policy change;
...
- update `templates/pwsh_command_pack/export/as-common-pwsh-command-pack/SKILL.md` and `scripts/install_pwsh_command_pack_skill.py` when the installable common skill package or installer guardrails change;
...
- update `scripts/migrate_artifact_names_4digit.py` and related tests when artifact migration rules, skip rules, collision behavior or dry-run/apply behavior change;
