...
| 790 | Post-MVP Roadmap and Hardening Plan | Consolidare warning, recovery drill, standard evidence e prossimi hardening del Motore ASF | Piano post-MVP con priorita', rischi residui e criteri readiness per uso ordinario | Post-MVP Motore | Completato |
| 800 | PowerShell Native Command Guardrail Hardening | Standardizzare controlli exit code, argomenti vuoti, `PrNumber`, `expected_files` e stop reale nei wrapper PowerShell | Publish runner, guardrail nativi PowerShell, test e regole command-pack aggiornate | Post-MVP Hardening | Completato |
| 805 | PowerShell Publish Skill Sync With Proven ASF Runner Flow | Allineare skill/template PowerShell al flusso config JSON + runner dimostrato dagli STEP 0790/0800 | Template, skill export, doc 0805 e test anti-regressione | Post-MVP Hardening | Completato |
| 810 | Publish Runner Scope Discovery, Recovery UX and No-False-Completed Guard | Aggiungere scope discovery, PrepareConfig, recovery out-of-scope e DOCX/accessori best-effort senza rendere il runner permissivo | Runner fail-closed piu' assistivo, template aggiornati, doc 0810 e test anti-regressione | Post-MVP Hardening | Completato |
| 820 | Bridge Output Consistency and LAST Validation | Validare coerenza tra Bridge, `LAST-*`, step, branch e artifact versionati | Regole di consistenza output Bridge e warning stale | Post-MVP Hardening | Proposto |
| 830 | MVP Real Step Pilot 4 - Slightly More Operational | Provare il Motore su uno step reale piccolo ma piu' operativo | Pilot con evidence post-MVP e warning dichiarati | Post-MVP Pilot | Proposto |
| 840 | Runner Hook Evidence Manifest Post-Publish Pack | Consolidare manifest hook-aware dopo pubblicazione reale human-gated | Evidence pack post-publish runner -> state machine -> manifest | Post-MVP Hardening | Proposto |
