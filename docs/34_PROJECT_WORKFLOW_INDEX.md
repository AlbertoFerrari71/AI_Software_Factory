...
| Gestire warning Git LF/CRLF nel publish runner | `docs/motor/0923_PUBLISH_RUNNER_LF_CRLF_WARNING_STDERR_HANDLING.md` | `scripts/asf_publish_step.ps1`, `tests/unit/test_asf_publish_step_lf_crlf_warning_handling.py` | Quando `git diff --check` o path discovery Git scrivono warning LF/CRLF su stderr con exit code 0 | `stderr warning treated as non-blocking`, `LF will be replaced by CRLF`, `CRLF will be replaced by LF`, `Git command wrote unexpected stderr`, exit code non zero bloccante e prossimo passo `0930) External Repo Push Pattern Generalization` |
